...
Medication safeguards keep a drug class distinct from a specific product, preserve strength and formulation, distinguish prescribed medication from medication actually being taken, and keep scheduled therapy separate from PRN use. The model does not convert a generic ingredient to an ambiguous brand, guess sound-alike names, drop ingredients from combination products, or treat a discontinued medicine as current. An adverse effect alone supports neither Yes nor No for an allergy field; allergy status remains blank unless explicitly documented. Ambiguous medication identities remain blank.
