...
async function fillPdf(pdfArrayBuffer, fieldMap) {
  // Load a fresh copy for filling
  const pdfDoc = await PDFDocument.load(new Uint8Array(pdfArrayBuffer), { ignoreEncryption: true });
  ...
  for (const field of fields) {
    ...
    try {
      ...
      console.warn(`Skipping field "${fieldName}":`, e.message);
      skippedFields.push(fieldName);
    }
  }
...
You will receive the PDF form visually, a list of all PDF field names with their types, and clinical notes.
Map the clinical information to the correct fields by reading the visual form layout.

Rules:
- For TextField: provide a string value
- For CheckBox: provide true or false (boolean)
- For RadioGroup: provide exactly one of the listed options as a string
- For fields you cannot fill: use null

Respond ONLY with a valid JSON object. Keys must be EXACT field names. No markdown, no explanation.`;
      ...
      setError(err.message.includes("JSON")
        ? "Unexpected AI response format. Please try again."
        : `Error: ${err.message}`);
    } finally {
      setLoading(false);
    }
